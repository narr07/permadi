...
async function interestByRegion({
  keyword, geo = 'ID', resolution = 'REGION', hl = 'id-ID',
...
async function relatedTopics({
  keyword, geo = 'ID', startTime = new Date('2004-01-01'), endTime = new Date(),
  category = 0, hl = 'id-ID', enableBackoff = false,
} = {}) {
  if (!keyword) throw new Error('Keyword is required');
    ...
    switch (command) {
      ...
      case 'explore':
        ...
        result = await explore({
          keyword, geo: parsed.geo || 'ID', time: parsed.time || 'now 1-d',
          category: parseInt(parsed.category || '0', 10), property: parsed.property || '', hl: parsed.hl || 'id-ID',
      ...
      case 'interest-by-region':
        ...
        result = await interestByRegion({
          keyword, geo: parsed.geo || 'ID', resolution: parsed.resolution || 'REGION', hl: parsed.hl || 'id-ID',
      ...
      case 'related-topics':
        ...
        result = await relatedTopics({
          keyword, geo: parsed.geo || 'ID', hl: parsed.hl || 'id-ID',
          startTime: parsed['start-time'] ? new Date(parsed['start-time']) : new Date('2004-01-01'),
          endTime: parsed['end-time'] ? new Date(parsed['end-time']) : new Date(),
          category: parseInt(parsed.category || '0', 10), enableBackoff: !!parsed['enable-backoff'],
        });
