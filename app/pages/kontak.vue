<script setup lang="ts">
	...
	const { data: page } = await useAsyncData(
		() => 'kontak-page-id',
		() => queryCollection('pages_id').path('/id/kontak').first()
	)

	useSeoMeta({
		title: computed(() => page.value?.title || 'Kontak — Dinar Permadi Yusup'),
	...
	<div class="container-bento py-10 sm:py-14">
		<header class="max-w-3xl mb-10">
			<span class="badge-neutral text-brand-600 dark:text-brand-400 font-semibold mb-3">
				<span class="i-lucide-message-square text-xs mr-1 inline-block" /> Kontak & Diskusi
			</span>
