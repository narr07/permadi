<script setup lang="ts">
	...
	useSeoMeta({
		title: computed(() => page.value?.title || 'Projects — Dinar Permadi Yusup'),
	...
	<div class="container-bento py-10 sm:py-14">
		...
		<header class="max-w-3xl mb-8 sm:mb-12">
			<span class="badge-neutral text-brand-600 dark:text-brand-400 font-semibold mb-3">
				<span class="i-lucide-folder-git-2 text-xs mr-1 inline-block" /> Selected Work
			</span>
			<h1 class="heading-hero text-slate-900 dark:text-white">
				{{ page?.title || 'Projects & Case Studies' }}
			</h1>
		...
		<EmptyState
			...
			:actions="[{ label: 'Lihat Semua Projek', to: localePath(locale === 'id' ? '/projek' : '/projects') }]"
		/>
	</div>
</template>
