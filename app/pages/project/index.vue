...
const { data: allProject } = await useAsyncData(`allproject-${route.path}`, async () => {
  ...
  return await queryCollection(collection)
    .select('title', 'description', 'path', 'id', 'date', 'meta')
    .order('date', 'DESC')
        ...
        >
          <NuxtLink :to="localePath(`/project${project.path}`)" class="group flex flex-col focus:outline-none">
            <div class="relative pt-[56.25%] rounded-xl overflow-hidden">
              <NuxtImg class="size-full absolute top-0 start-0 object-cover group-hover:scale-105 group-focus:scale-105 transition-transform duration-500 ease-in-out rounded-xl" :src="project.image" :alt="project.title" />
            ...
            <div class="mt-7">
              <h2 class="text-xl font-semibold text-gray-800 group-hover:text-gray-600 dark:text-neutral-300 dark:group-hover:text-white">
                {{ project.title }}
              </h2>
            </div>
          </NuxtLink>
