...
<template>
	...
	>
		...
		<div class="grid grid-cols-2 gap-2.5 md:grid-cols-6 sm:grid-cols-3">
			<SocialShare
				...
				:url="url || undefined"
				user="dinarpermadi07"
				class="group relative flex items-center justify-center gap-2.5 border border-slate-200/70 rounded-xl bg-slate-50 px-3 py-2.5 text-xs text-slate-700 font-semibold transition-all duration-200 sm:justify-start dark:border-[#134e43]/90 dark:bg-[#002420]/80 hover:bg-white dark:text-slate-300 hover:shadow-sm focus-ring hover:-translate-y-0.5 dark:hover:bg-[#003833]"
				:class="net.hoverClass"
			>
				<span
					:class="net.icon"
					class="shrink-0 text-sm transition-transform duration-200 group-hover:scale-110"
				/>
				<span class="truncate font-medium font-sans">{{ net.label }}</span>
			</SocialShare>
		</div>
	</aside>
</template>
