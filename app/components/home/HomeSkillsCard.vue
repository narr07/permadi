...
<template>
	<div class="flex flex-col justify-between gap-4 md:col-span-5 sm:gap-5">
		<!-- Card 1: Programming -->
		<div class="bento-card-clean flex flex-1 flex-col justify-between p-5 sm:p-6">
			<div class="grid grid-cols-1 items-center gap-4 sm:grid-cols-12 sm:gap-5">
				<!-- Sisi Kiri: Context & Judul -->
				<div class="flex flex-col justify-between sm:col-span-7">
					<div>
						<h3 class="text-lg text-slate-900 font-bold leading-tight font-heading sm:text-xl dark:text-white">
							{{ skillsData.code_title || 'Programming' }}
						</h3>
						<p class="mt-1.5 text-xs text-slate-700 leading-relaxed dark:text-slate-300">
					...
					<div class="mt-3 flex items-center gap-1.5 text-[11px] text-slate-500 font-mono dark:text-slate-400">
				...
				<div class="flex flex-col gap-2 sm:col-span-5 sm:items-end">
					<div
						v-for="skill in codeSkills"
						:key="skill.name"
						class="w-full flex items-center gap-2.5 border border-slate-200/70 rounded-xl bg-slate-50/80 px-2.5 py-2 transition-all sm:w-[155px] dark:border-slate-800/70 hover:border-brand-500/40 dark:bg-slate-800/40 dark:hover:border-brand-400/40"
					>
						<img
						...
						/>
						<span class="truncate text-xs text-slate-900 font-semibold font-heading dark:text-white">
							{{ skill.name }}
						</span>
					</div>
				</div>
