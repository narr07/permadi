		...
		<div class="grid grid-cols-1 lg:grid-cols-12">
			...
			<div class="flex flex-col justify-between border-b border-slate-200/80 bg-slate-50/40 p-6 lg:col-span-5 lg:border-b-0 lg:border-r dark:border-[#134e43] dark:bg-[#002420]/30 lg:p-10 sm:p-8">
				<div>
					...
					<p class="max-w-[44ch] text-xs text-slate-900/75 leading-relaxed font-sans sm:text-sm dark:text-slate-50/75">
			...
			<div class="flex flex-col justify-between p-6 lg:col-span-7 lg:p-10 sm:p-8">
					...
					<div class="grid grid-cols-1 gap-8 sm:grid-cols-2 divide-y divide-slate-200/80 sm:divide-x sm:divide-y-0 dark:divide-[#134e43]">
						...
						<div class="sm:pr-6">
							...
							<ul
								...
								class="text-xs text-slate-900/80 font-sans space-y-3 sm:text-sm dark:text-slate-50/80"
							>
								<li
								...
								>
									<span class="pt-0.5 text-[10px] text-brand-500 font-bold font-mono">0{{ idx + 1 }}.</span>
									<div>
										<strong class="text-slate-900 font-semibold dark:text-slate-50">{{ item.name }}</strong>
										<span
											v-if="item.desc"
											class="block text-xs text-slate-900/60 dark:text-slate-50/60"
										>{{ item.desc }}</span>
									</div>
								</li>
							</ul>
						...
						<div class="pt-6 sm:pl-6 sm:pt-0">
							<div class="mb-4">
								<span class="text-[11px] text-[#f9bc60] font-bold tracking-widest font-mono uppercase">
							...
							>
								<li
									...
									class="flex items-start gap-2.5"
								>
									<span class="pt-0.5 text-[10px] text-[#f9bc60] font-bold font-mono">0{{ idx + 1 }}.</span>
									<div>
										<strong class="text-slate-900 font-semibold dark:text-slate-50">{{ item.name }}</strong>
										<span
											v-if="item.desc"
				...
				<div class="mt-8 flex items-center justify-between border-t border-slate-200/80 pt-4 text-[11px] text-slate-900/50 font-mono dark:border-[#134e43] dark:text-slate-50/50">
