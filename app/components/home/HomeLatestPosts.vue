...
<template>
	<div class="bento-card-clean md:col-span-12 p-6 sm:p-8">
		<div class="flex items-center justify-between mb-6 pb-3.5 border-b border-slate-200/60 dark:border-slate-800/60">
			<div>
				<span class="section-label text-brand-700 dark:text-brand-400 block mb-1">
			...
			<NuxtLink
				:to="localePath('/blog')"
				class="text-xs font-bold text-brand-800 dark:text-brand-400 hover:text-brand-950 dark:hover:text-yellow-600 hidden sm:inline-flex items-center gap-1 transition-colors"
			>
		...
		<div v-if="posts?.length" class="divide-y divide-slate-100 dark:divide-slate-800/60">
			...
			>
				<div class="flex items-center gap-3 min-w-0">
					<span class="text-xs font-mono font-bold text-brand-700 dark:text-brand-300 w-6 shrink-0 group-hover:text-brand-950 dark:group-hover:text-yellow-600 transition-colors">
						0{{ pIdx + 1 }}
					</span>
					<h4 class="font-heading font-semibold text-base sm:text-lg text-brand-950 dark:text-brand-100 group-hover:text-brand-800 dark:group-hover:text-yellow-600 transition-colors duration-150 truncate">
						{{ post.title }}
					</h4>
				</div>
				<div class="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400 shrink-0 sm:pl-4">
					...
					<span class="i-hugeicons-arrow-right-01 text-xs text-brand-700 dark:text-yellow-600 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
				</div>
			</NuxtLink>
		</div>
		<p v-else class="text-slate-600 dark:text-slate-400 text-xs py-4">
